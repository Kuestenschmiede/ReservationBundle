<?php
/*
 * This file is part of con4gis, the gis-kit for Contao CMS.
 * @package con4gis
 * @version 10
 * @author con4gis contributors (see "authors.txt")
 * @license LGPL-3.0-or-later
 * @copyright (c) 2010-2025, by Küstenschmiede GmbH Software & Design
 * @link https://www.con4gis.org
 */

$GLOBALS['TL_LANG']['tl_c4g_reservation_event_participants']['id'] = array("#", "");
$GLOBALS['TL_LANG']['tl_c4g_reservation_event_participants']['organisation'] = 'Company / Organization / School';
$GLOBALS['TL_LANG']['tl_c4g_reservation_event_participants']['salutation'] = 'Salutation';
$GLOBALS['TL_LANG']['tl_c4g_reservation_event_participants']['man'] = 'Mr.';
$GLOBALS['TL_LANG']['tl_c4g_reservation_event_participants']['woman'] = 'Mrs.';
$GLOBALS['TL_LANG']['tl_c4g_reservation_event_participants']['various'] = ' - ';
$GLOBALS['TL_LANG']['tl_c4g_reservation_event_participants']['title']  = array("Title", "Title");
$GLOBALS['TL_LANG']['tl_c4g_reservation_event_participants']['lastname'] = array("Last name", "Last name");
$GLOBALS['TL_LANG']['tl_c4g_reservation_event_participants']['firstname'] = array("First name", "First name");
$GLOBALS['TL_LANG']['tl_c4g_reservation_event_participants']['dateOfBirth'] = array("Date of Birth", "Date of Birth");
$GLOBALS['TL_LANG']['tl_c4g_reservation_event_participants']['email'] = array("Email address", "First name");
$GLOBALS['TL_LANG']['tl_c4g_reservation_event_participants']['phone'] = array("Phone number", "Phone number");
$GLOBALS['TL_LANG']['tl_c4g_reservation_event_participants']['address'] = array("Street", "Street");
$GLOBALS['TL_LANG']['tl_c4g_reservation_event_participants']['postal'] = array("Postal code", "Postcode");
$GLOBALS['TL_LANG']['tl_c4g_reservation_event_participants']['city'] = array("City", "Town");
$GLOBALS['TL_LANG']['tl_c4g_reservation_event_participants']['comment'] = array("comment", "comment from creator");
$GLOBALS['TL_LANG']['tl_c4g_reservation_event_participants']['reservation_participant_option'] = array("Participant services", "Selected participant options");
$GLOBALS['TL_LANG']['tl_c4g_reservation_event_participants']['cancellation'] = array("Canceled", "If the appointment is cancelled, select this field");
$GLOBALS['TL_LANG']['tl_c4g_reservation_event_participants']['checkedIn'] = array('Person is on site (check-in).','');
$GLOBALS['TL_LANG']['tl_c4g_reservation_event_participants']['payed'] = array('Reservation has been paid.','');

$GLOBALS['TL_LANG']['tl_c4g_reservation_event_participants']['yes'] = 'ja';
$GLOBALS['TL_LANG']['tl_c4g_reservation_event_participants']['no'] = 'no';

$GLOBALS['TL_LANG']['tl_c4g_reservation_event_participants']['booked_by'] = 'Booked by';
$GLOBALS['TL_LANG']['tl_c4g_reservation_event_participants']['reservation_id'] = 'Reservation Number';

/** LEGENDS **/
$GLOBALS['TL_LANG']['tl_c4g_reservation_event_participants']['participants_legend'] = "Participant Data";

/** OPERATIONS **/
$GLOBALS['TL_LANG']['tl_c4g_reservation_event_participants']['new'] = array("Add participant","Add participant");
$GLOBALS['TL_LANG']['tl_c4g_reservation_event_participants']['edit'] = array("Edit participants","Edit participants ID %s");
$GLOBALS['TL_LANG']['tl_c4g_reservation_event_participants']['copy'] = array("Copy participants","Copy participants ID %s");
$GLOBALS['TL_LANG']['tl_c4g_reservation_event_participants']['delete'] = array("Delete participants","Delete participants ID %s");
$GLOBALS['TL_LANG']['tl_c4g_reservation_event_participants']['show'] = array("Display participants","Display participants ID %s");
$GLOBALS['TL_LANG']['tl_c4g_reservation_event_participants']['TOGGLE'] = array("Activate participants","Activate participants ID %s");
